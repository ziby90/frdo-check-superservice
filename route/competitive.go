...
func AddCompetitiveGroupsHandler(r *mux.Router) {
	...
	// просто список конкурсов для выбора при подаче заявления
	r.HandleFunc("/competitive/short", func(w http.ResponseWriter, r *http.Request) {
		var res handlers.ResultInfo
		keys := r.URL.Query()
		res.User = *handlers.CheckAuthCookie(r)
		// TODO ограничение на пять вузов
		res.GetShortListCompetitiveGroups(keys)
		service.ReturnJSON(w, &res)
	}).Methods("GET")
	// просто список конкурсов для выбора при подаче заявления
	...
	// изменение комментария конкурсной группы
	r.HandleFunc("/competitive/{id:[0-9]+}/comment/edit", func(w http.ResponseWriter, r *http.Request) {
		var res handlers.ResultInfo
		var cmp struct {
		...
		res.EditCompetitiveComment(uint(id), cmp.Comment)
		service.ReturnJSON(w, &res)
	}).Methods("Post")
	// изменение названия конкурсной группы
	r.HandleFunc("/competitive/{id:[0-9]+}/name/edit", func(w http.ResponseWriter, r *http.Request) {
		var res handlers.ResultInfo
		var cmp struct {
		...
		res.EditCompetitiveName(uint(id), cmp.Name)
		service.ReturnJSON(w, &res)
	}).Methods("Post")
	...
	r.HandleFunc("/competitive/entrance/{id:[0-9]+}/date/add", func(w http.ResponseWriter, r *http.Request) {
		var res handlers.ResultInfo
		var cmp handlers.AddEntranceTestDate
		b, _ := ioutil.ReadAll(r.Body)
		err := json.Unmarshal(b, &cmp)
		vars := mux.Vars(r)
		res.User = *handlers.CheckAuthCookie(r)
